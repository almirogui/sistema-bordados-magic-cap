 ...
 * - Envia email de confirmação para cliente (inglês)
 * - Envia notificação para admin (português)
 *
...
class Bordados_Hook_Criacao_Pedido {
    ...
    public function __construct() {
        // Hook após criar pedido - prioridade 10 para atribuição
        add_action('bordados_pedido_criado', array($this, 'verificar_atribuicao_automatica'), 10, 2);
        
        // Hook após criar pedido - prioridade 20 para emails (roda depois da atribuição)
        add_action('bordados_pedido_criado', array($this, 'enviar_emails_confirmacao'), 20, 2);
    }

    /**
     * NOVO: Enviar emails de confirmação quando pedido é criado
     *
     * @param int $pedido_id ID do pedido criado
     * @param int $cliente_id ID do cliente
     */
    public function enviar_emails_confirmacao($pedido_id, $cliente_id) {
        // Verificar se classe de emails existe
        if (!class_exists('Bordados_Emails')) {
            error_log("Bordados: Classe Bordados_Emails não encontrada. Emails de confirmação não serão enviados.");
            return false;
        }
        
        // Log
        error_log("Bordados: Enviando emails de confirmação para pedido #{$pedido_id}");
        
        // 1. Email de confirmação para o CLIENTE (inglês)
        try {
            $enviado_cliente = Bordados_Emails::enviar_confirmacao_pedido_cliente($pedido_id);
            if ($enviado_cliente) {
                error_log("Bordados: ✅ Email de confirmação enviado para cliente (pedido #{$pedido_id})");
            }
        } catch (Exception $e) {
            error_log("Bordados: ERRO ao enviar email para cliente: " . $e->getMessage());
        }
        
        // 2. Notificação para o ADMIN (português)
        try {
            $enviado_admin = Bordados_Emails::notificar_admin_novo_pedido($pedido_id);
            if ($enviado_admin) {
                error_log("Bordados: ✅ Notificação enviada para admin (pedido #{$pedido_id})");
            }
        } catch (Exception $e) {
            error_log("Bordados: ERRO ao enviar notificação para admin: " . $e->getMessage());
        }
